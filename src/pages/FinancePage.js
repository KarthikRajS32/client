...
const FinancePage = () => {
  const [goals, setGoals] = useState([]);
  const [currentGoal, setCurrentGoal] = useState({ name: '', target: '', deadline: '' });
  const [savingsInputs, setSavingsInputs] = useState({}); // State to track savings inputs
  ...
  const handleAddGoal = () => {
    ...
    setCurrentGoal({ name: '', target: '', deadline: '' });
    setSavingsInputs({ ...savingsInputs, [newGoal.id]: '' }); // Initialize savings input for the new goal
    message.success('Financial goal added successfully!');
  };
  ...
  const handleAddSavings = (id) => {
    ...
    setGoals(updatedGoals);
    setSavingsInputs({ ...savingsInputs, [id]: '' }); // Clear the input field after adding savings
    message.success('Savings added successfully!');
  };
